  ...
  AI_DETECTED  - Strong indicators of AI generation (repetition, collapse, etc.)
    ...
    if args.folder:
        ...
        if not args.quiet:
            ...
            print("Summary:")
            print(f"  ✅ AUTHENTIC:   {verdicts.get('AUTHENTIC', 0)}")
            print(f"  ⚠️  SUSPICIOUS:  {verdicts.get('SUSPICIOUS', 0)}")
            print(f"  🤖 AI_DETECTED: {verdicts.get('AI_DETECTED', 0)}")
            ...
            if args.json:
                ...
                if flagged:
                    ...
                    for r in flagged:
                        icon = "🤖" if r.get("verdict") == "AI_DETECTED" else "⚠️"
